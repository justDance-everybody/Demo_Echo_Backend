...
### 数据库配置策略
**默认**：开发环境默认使用SQLite（`sqlite:///app.db`）。

- **生产建议**：生产环境建议配置MySQL，通过 `.env` 的 `DATABASE_URL` 指定，例如 `mysql+pymysql://user:pass@host:3306/db`。
- **连接检查**：在安装与启动脚本中提供连通性检查（可选）。
- **配置文件**：所有配置统一通过 `Backend/.env` 管理。
...
class Settings(BaseSettings):
    ...
    def __init__(self, **data):
        super().__init__(**data)
        # 生产环境建议 MySQL：通过环境变量覆盖，无强制改写

settings = Settings()
```

### .env 文件示例（Backend/.env）
```
# 应用基础配置
...
当前版本的 API 集成功能在对接 Dify 平台时，存在以下限制：

1.  **仅支持工作流（Workflow）模式**：系统目前仅支持调用 Dify 的工作流类型应用。在配置和测试 Dify 集成时，请确保您提供的 `base_url` 指向一个工作流应用。系统会默认将请求发送到 `{base_url}/workflows/run`。

2.  **明确的错误反馈**：如果您尝试连接一个非工作流类型的 Dify 应用（例如 Chat App），`validate-and-test` 接口将会返回一个明确的错误信息，提示“The connected Dify application does not appear to be a workflow.”。这旨在帮助您快速定位问题。

此设计是为了在当前阶段简化集成逻辑，后续版本会根据需求考虑支持更多 Dify 应用类型。
